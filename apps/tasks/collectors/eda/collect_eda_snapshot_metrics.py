"""
Snapshot metrics collector for EDA (Event-Driven Automation).

Collects daily snapshots from the EDA PostgreSQL database using the
``ms_aap_readonly`` read-only account. If the EDA DB is unreachable
(credentials missing or host not configured), returns a structured error
result without raising — so the EDA_COLLECTION_GROUP fails cleanly
...
from ._registry import _get_eda_collectors

logger = logging.getLogger(__name__)


def collect_eda_snapshot_metrics(**kwargs: Any) -> dict[str, Any]:
    """Collect a snapshot from the EDA database.

    Args:
        **kwargs: Task data containing:
            - collector_type (str): One of the keys in the EDA collector registry
              (e.g., ``'eda_config'``, ``'eda_activations'``).
    ...
    registry = _get_eda_collectors()
    if collector_type not in registry:
        valid = ", ".join(sorted(registry.keys()))
        return create_task_result("error", error=f"Unknown EDA collector_type: '{collector_type}'. Valid: {valid}")
    ...
    try:
        db_connection = get_db_connection("eda")
    except Exception as e:
        logger.warning("collect_eda_snapshot_metrics: cannot connect to EDA DB: %s", e)
        return create_task_result(
            "error",
            {"collector_type": collector_type, "source": "eda"},
            error=f"EDA database connection failed: {e}",
        )
